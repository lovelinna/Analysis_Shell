...
prepaire_for_task(){
	...
	#		echo "date="$date" newdate="$new_date
				...
				tzmap["$key"]="$new_date"
                                ...
                                campaigns=`$MYSQL -e "select count(*) from at_campaign where (activate_time >= "$activate_timestamp" and activate_time < "$expire_timestamp") or (activate_time < "$activate_timestamp" and expire_time<="$activate_timestamp")" 2>/dev/null|awk 'NR>1'`
